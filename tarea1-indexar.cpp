...
Mat vector_de_intensidades(string nombre_imagen, string imagenes_dir) {
  ...
  // resize(imagen_reducida, imagen_reducida_grande, Size(imagen.cols, imagen.rows), 0, 0, INTER_NEAREST);
  // mostrar_imagen("Imagen reducida", imagen_reducida_grande, false, false);
  // cout << "Vector de intensidades: " << vector_intensidades << endl;
  // waitKey(0);
...
void tarea1_indexar(const char *dir_input_imagenes_R,
  ...
  vector<std::string> imagenes = listar_archivos_en_carpeta(dir_input_imagenes_R);
  Mat descriptores = calcular_descriptores(vector_de_intensidades, imagenes, dir_input_imagenes_R);
  cout << "Descriptores: " << endl << descriptores << endl;
  cout << "Filas: " << descriptores.rows << " Columnas: " << descriptores.cols << endl;
}
